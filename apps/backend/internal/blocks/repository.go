...
func UpdateBlock(block *Block, ctx context.Context) error {
	_, err := foundation.PostgresDB.ExecContext(ctx, "UPDATE blocks SET document_id = $1, order_path = $2, type = $3, content = $4, updated_at = $5, updated_by = $6 WHERE id = $7", block.DocumentID, pq.Array(block.OrderPath), block.BlockType, block.Content, block.UpdatedAt, block.UpdatedBy, block.ID)
	return err
}

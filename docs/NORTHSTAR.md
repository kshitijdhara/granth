...
- ~~**Conflict detection is frontend-only; the backend does not enforce.**~~ **Partially closed 2026-05-29** — `acceptProposal()` now queries open proposals sharing `affected_block_ids` via the GIN index and blocks acceptance if a conflict exists. **Remaining gap:** this only catches proposals that are *simultaneously open*. If Proposal A is accepted first and then Proposal B (which was written against the same original block content) is accepted, B's `UPDATE` runs unconditionally and silently overwrites A's changes. No content is preserved; no warning is surfaced. This is the sequential-accept content-loss problem. Fix requires storing `old_content` / `old_block_type` in `proposal_block_changes` at proposal-creation time, then verifying at accept time that the block still matches that snapshot before applying the change. If it doesn't match, the proposal is stale and must be explicitly re-based or rejected. See §15 item 9b and the 2026-05-29 decision log entry.
...
9. ~~**Backend conflict enforcement (simultaneous open proposals).** At accept time, query open proposals sharing `affected_block_ids` using the existing GIN index. Surface the conflict as a blocking warning or require explicit override. Frontend detection alone is insufficient.~~ ✓ **Completed 2026-05-29**
9b. **Stale-proposal content-loss prevention (sequential acceptance).** Add `old_content TEXT` and `old_block_type TEXT` to `proposal_block_changes` (captured at proposal-creation time for `update` actions). At accept time, before applying each `update` change, verify the block's current content matches `old_content`. If it doesn't, a previously accepted proposal has already moved the block — surface this as a blocking error requiring explicit re-base or rejection. Without this, accepting two proposals that both modify the same block in sequence silently discards the first accepted change. This is the canonical "lost update" problem. See §14 and the 2026-05-29 decision log entry.
...
- **Decision (2026-05-29):** Identified sequential-accept content-loss gap; backend conflict enforcement marked partially closed.
  **Reasoning:** `acceptProposal()` now blocks acceptance when a *simultaneously open* proposal touches the same blocks. However, the check only inspects `state = 'open'` proposals. Once Proposal A is accepted, Proposal B (written against the same original block content) sees no conflict and applies its `UPDATE` unconditionally — silently overwriting A's changes. The root cause is that `proposal_block_changes` records only the *intended* new content, not the *base* content the proposal was written against. Without a base snapshot, there is no way to detect at accept time that the block has moved. The fix (§15 item 9b) is a snapshot-on-create / verify-on-accept pattern, equivalent to Git's base-commit check. Until it is implemented, the system is safe against simultaneous conflicts but not against sequential ones.
  **Tradeoffs accepted:** The gap is narrow in practice — it requires two proposals for the same block to both reach acceptance, with the first accepted before the second is reviewed. For most workspaces this is rare enough to defer. Flagged now so it is not forgotten.

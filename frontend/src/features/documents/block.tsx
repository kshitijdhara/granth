    ...
    return (
      <div className="block block--editing" style={indent}>
        {block.block_type === "heading" ? (
          <input type="text" value={block.content} onChange={handleChange} onBlur={handleBlur}
            className="block__input block__input--heading" placeholder="Heading" />
        ) : block.block_type === "code" ? (
          <textarea value={block.content} onChange={handleChange} onBlur={handleBlur}
            className="block__textarea block__textarea--code" rows={8} placeholder="Code" />
        ) : (
          <textarea value={block.content} onChange={handleChange} onBlur={handleBlur}
            className="block__textarea block__textarea--text" rows={4} placeholder="Text" />
        )}
            ...
            <div className="block__add-menu" ref={menuRef}>
              <div>
                ...
                {(["text", "heading", "code"] as const).map((type) => (
                  <button key={type} onClick={() => { onAddBlock?.(null, type); setShowMenu(false); }} disabled={isAdding}>
              ...
              <div>
                ...
                {(["text", "heading", "code"] as const).map((type) => (
                  <button key={type} onClick={() => { onAddBlock?.(block.id, type); setShowMenu(false); }} disabled={isAdding}>
                    {type.charAt(0).toUpperCase() + type.slice(1)}
                  </button>
                ))}
              </div>

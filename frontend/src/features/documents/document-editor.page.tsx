  ...
  useEffect(() => {
    ...
    Promise.all([documentsApi.get(id), blocksApi.getAll(id).catch(() => [])])
      .then(([doc, blks]) => {
        setDocument(doc);
        setTitle(doc.title ?? "");
        setBlocks((blks as Block[]).sort((a, b) => compareOrderPaths(a.order_path ?? [], b.order_path ?? [])));
      })
  ...
  const handleCreateProposal = async (data: { title: string; intent: string; scope: string }) => {
    ...
    try {
      const affectedBlockIds = [...new Set(
        changes.filter((c) => c.action !== "create").map((c) => c.block.id)
      )];
      ...
      if (parentId === null) {
        ...
        const children = blocks.filter(
          (b) =>
            ...
            b.order_path.slice(0, parent.order_path.length).every((v, i) => v === parent.order_path[i])
        );
        const maxChild = children.length > 0 ? Math.max(...children.map((b) => b.order_path[parent.order_path.length])) : 0;
      ...
      <div className="document-editor">
        <header className="document-editor__header">
          ...
          <div className="document-editor__actions">
            <Button variant="secondary" size="small" onClick={() => navigate(`/documents/${id}`)} isFullWidth={false}>
              Cancel
            </Button>
            <Button
              variant="primary"

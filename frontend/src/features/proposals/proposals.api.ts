...
export const proposalsApi = {
  getForDocument: (documentId: string) =>
    http.get<Proposal[]>(`/proposals/document/${documentId}`),

  create: (documentId: string, data: {
    title: string;
    intent: string;
    scope: string;
    affected_block_ids: string[];
  ...
  update: (id: string, data: { title: string; intent: string; scope: string; affected_block_ids: string[] }) =>
  ...
  addBlockChange: (proposalId: string, data: {
    block_id: string | null;
    action: string;
    block_type: string;
    order_path: number[];

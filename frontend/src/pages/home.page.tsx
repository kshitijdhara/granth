  ...
  useEffect(() => {
    documentsApi.getAll()
      .then((docs) => setDocuments(
        docs.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).slice(0, 10)
      ))
      ...
      <div className="home-page__container">
        ...
        <main className="home-page__content">
          <div className="home-page__actions">
            ...
            <Button variant="secondary" size="medium" onClick={() => navigate("/documents")} isFullWidth={false}>
              All Documents
            </Button>
          </div>
          ...
          <section className="home-page__documents">
            ...
            {loading ? (
              <div className="home-page__skeleton-grid">
                {Array.from({ length: 6 }).map((_, i) => <div key={i} className="home-page__skeleton-card" />)}
              </div>
            ) : documents.length === 0 ? (
              ...
              <div className="home-page__documents-grid">
                {documents.map((doc) => (
                  <Card key={doc.id} variant="default" padding="md"
                    ...
                    <div className="home-page__document-icon"><DocumentTextIcon /></div>
                    <h3 className="home-page__document-title">{doc.title}</h3>
                    <p className="home-page__document-meta">{relativeDate(doc.updated_at || doc.created_at)}</p>
                  </Card>
                ))}
              </div>
            )}
